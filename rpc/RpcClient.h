...
/// 3. 适配 ClientStub _onNewConnection 的逻辑：在 RpcClient 中先创建好
...
private:
  ...
  static RpcClient *s_rpcClient;
};

#define RPC_CLIENT ::lrpc::RpcClient::instance()
...
Future<Result<R>> call(const std::string &service, const std::string &method,
                       ...
                       const Endpoint &ep = Endpoint::default_instance()) {
  // 找到 clientStub
  auto stub = RPC_CLIENT.getClientStub(service);
  if (!stub)
