...
// helpers
static const char *Strstr(const char *ptr, size_t nBytes, const char *pattern,
                          size_t nBytes2) {
...
static const char *SearchCRLF(const char *ptr, size_t nBytes) {
  return Strstr(ptr, nBytes, CRLF, 2);
}

static ParseResult GetIntUntilCRLF(const char *&ptr, std::size_t nBytes,
                                   int &val) {
  if (nBytes < 3)
...
ParseResult ServerProtocol::_ParseMulti(const char *&ptr, const char *end,
                                        int &result) {
  if (end - ptr < 3)
    return ParseResult::wait;
...
ParseResult ServerProtocol::_ParseStrlist(const char *&ptr, const char *end,
...
ParseResult ServerProtocol::_ParseStr(const char *&ptr, const char *end,
                                      std::string &res) {
  if (paramLen_ == -1) {
    const char *const start = ptr;
...
ParseResult ServerProtocol::_ParseStrval(const char *&ptr, const char *end,
                                         std::string &res) {
  assert(paramLen_ >= 0);
...
ParseResult ServerProtocol::_ParseStrlen(const char *&ptr, const char *end,
                                         int &result) {
  if (end - ptr < 3)
    return ParseResult::wait;
...
// ClientProtocol
ClientProtocol::ClientProtocol() { Reset(); }
...
ParseResult ClientProtocol::_ParseStrlen(const char *&ptr, const char *end,
                                         int &result) {
  if (end - ptr < 3)
    return ParseResult::wait;
